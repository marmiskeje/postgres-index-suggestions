...
{
    class LoadDebugTreesToContextCommand : ChainableCommand
    {
        ...
        private readonly Func<IEnumerable<string>> getInputFunc;
        private readonly Func<ICollection<JObject>> getOutputCollectionFunc;
        public LoadDebugTreesToContextCommand(Func<IEnumerable<string>> getInputFunc, Func<ICollection<JObject>> getOutputCollectionFunc)
        {
            this.getInputFunc = getInputFunc;
            this.getOutputCollectionFunc = getOutputCollectionFunc;
        ...
        {
            var trees = getInputFunc();
            foreach (var tree in trees)
            {
                string inputParseTreeStr = tree;
                ...
                for (int i = 0; i < inputEntries.Count; i++)
                {
                    ...
                    else if (input.StartsWith("(("))
                    {
                        ...
                        var val = input.Substring(2, length);
                        bool appendComma = true;
                        if (val.StartsWith("{"))
                        {
                            input = "{ " + ConvertToJsonProperty(val.Substring(1)) + ": {";
                            appendComma = false;
                        }
                        else
                        {
                            input = ConvertToJsonProperty(val);
                        }
                        input = "[[ " + input;
                        if (originalInput.EndsWith(")"))
                        {
...
#endif
                var outputCollection = getOutputCollectionFunc();
