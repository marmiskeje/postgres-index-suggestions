...
namespace DiplomaThesis.WorkloadAnalyzer
{
    internal class PermuteUtils
    {
        ...
        // of the input.
        public static IEnumerable<IEnumerable<T>> Permute<T>(IEnumerable<T> list, int tupleLength)
        {
            if (tupleLength == 0)
            {
            ...
            else
            {
                ...
                foreach (T startingElement in list)
                {
                    ...
                    foreach (IEnumerable<T> permutationOfRemainder in Permute(remainingItems, tupleLength - 1))
                    {
