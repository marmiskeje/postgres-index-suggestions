...
namespace IndexSuggestions.IndexAnalysis
{
    internal class LoadExistingIndicesCommand : ChainableCommand
    {
        ...
        public LoadExistingIndicesCommand(DesignIndicesContext context, IIndicesRepository indicesRepository)
        {
        ...
        protected override void OnExecute()
        {
            using (var scope = new DatabaseScope(context.Database.Name))
            {
                ...
                foreach (var i in existingIndices)
                {
                    ...
                    context.ExistingIndices.Add(new IndexDefinition(Convert(i.AccessMethod), relation, attributes, null));
                }
            }
        }
